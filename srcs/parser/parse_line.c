...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/11/02 19:46:14 by baudibert         #+#    #+#             */
/*   Updated: 2020/11/02 19:47:15 by baudibert        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_v_line(const char *line, t_list_iterators *iterators)
{
	...
	if ((sscanf(line, "v %f %f %f", &new->pos.x, &new->pos.y, &new->pos.z))
			!= 3)
		clean_exit("v line sscanf error");
	new->next = NULL;
...
void	parse_vt_line(const char *line, t_list_iterators *iterators)
{
	...
	if ((sscanf(line, "vt %f %f", &new->uv.x, &new->uv.y)) != 2)
		clean_exit("vt line sscanf error");
	new->next = NULL;
...
void	parse_vn_line(const char *line, t_list_iterators *iterators)
{
	...
	if (!(new = (t_vn_lst *)malloc(sizeof(t_vn_lst))))
		clean_exit("malloc error");
	if ((sscanf(line, "vn %f %f %f", &new->normals.x, &new->normals.y,
			&new->normals.z)) != 3)
		clean_exit("vn line sscanf error");
	new->next = NULL;
